...
# from export_data import ExportForm
...
class MainForm:
    def __init__(self, master):
        ...
        if var['RDpath'] is None:
            rdlbl = 'None selected.'
        else:
            rdlbl = var['RDpath']
        # each of the following pairs defines a form object (widget) and then places it on the form using the grid
        # method. the 'command=' option defines internal class functions that occur when a widget is activated (e.g.
        # a button is pressed)
    ...
    # function to create a new blank Reporting Database for use.  Utilizes SQL script loading and execution.
    def create_RD(self):
        path = tkinter.filedialog.asksaveasfilename(title="Choose Reporting Database to use:",
        ...
        try:
            ...
            tkinter.messagebox.showerror("Error", "Chosen file is in use by another process or user does not have "
        ...
        if path:
            ...
            if not os.path.isdir(var['SQLpath']):
                tkinter.messagebox.showwarning('SQL path not found.',
                                               'Default SQL script directory (/path/to/main.py/sql) not found. '
                                               'Please choose custom SQL script directory.')
            ...
            # checks if db has been initialized to spatialite and initializes if not.
            ...
            # the following loop opens sql statements from the 'sql' directory and execute them in the newly
            # created file
            script_list = [[os.path.join(sqldir, 'create_tables.sql'), 'Creating tables...'],
                           ...
                           [os.path.join(sqldir, 'create_qaqc.sql'),
                            'Creating QAQC views and inserting QAQC config data...']]
            success = True
            finished = False
    ...
    # starts the import process from a DIMA.  NOTE: This will currently fail in a non-windows environment due the lack
    # of odbc connection dirvers to MS Access files in Linux or Mac
    def choose_DIMA(self):
        if var['RDpath'] is not None:
            if os.path.isfile(var['RDpath']):
                path = tkinter.filedialog.askopenfilename(title="Choose DIMA to import data from:",
                ...
                if path:
                    ...
                    r = rows.fetchone()
                    if r[0] == '0':
                        delrecords = tkinter.messagebox.askyesno("Delete?",
                                                                 "Would you like to delete current data before import?")
                    else:
                        delrecords = False
                ...
                tkinter.messagebox.showerror("Error", "Current database path does not exist. "
                                                      "Please connect to valid database.")
        else:
            tkinter.messagebox.showerror("Error", "Database path not chosen. Please connect to "
                                                  "valid database or create a new one first.")
    ...
    def Export_RD(self):
        if var['RDpath'] is not None:
            ...
            tkinter.messagebox.showerror("Error", "Database path not chosen. Please connect to valid "
                                                  "database or create a new one first.")
    ...
    def SQLscript(self):
        if var['RDpath'] is not None:
            if os.path.isfile(var['RDpath']):
                ...
                elif not success and stmts is not None:
